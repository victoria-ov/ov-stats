...
pr_cols = ['number', 'user.login', 'body', 'created_at', 'updated_at', 'closed_at', 'merged_at', 'requested_reviewers',
...
# specify columns of interest
open_prs_cols = ['number', 'user.login', 'body', 'created_at', 'updated_at', 'requested_reviewers',
               'labels', 'head.label', 'head.ref', 'head.repo.name', 'head.repo.size', 'state']
...
# Calculate the duration until the PR was closed
closed_prs['days-until-closed'] = closed_prs['merged_at'] - closed_prs['created_at']

open_prs.to_csv('open_prs.csv')
closed_prs.to_csv('closed_prs.csv')